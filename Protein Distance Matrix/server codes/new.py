...
def pipeline(dir_path, batch_size, resizeto=64, n_hidden=500, n_iteration=100, learning_rate = 0.0001):
    ...
    n_inputs = int((resizeto**2/2)-(resizeto/2)) # input is flatten version of input matrix
    n_hidden = n_hidden
    n_outputs = n_inputs
    ...
    with tf.Session() as sess:
        ...
        for iteration in range(n_iterations):
            ...
            iteration_loss = []
            for i in range(int(n_pdbs/batch_size)+1):
                ...
                _, loss_val = sess.run([training_op, reconstruction_loss], feed_dict={X: list(features)}) # no labels (unsupervised)
        ...
        for i in range(int(n_pdbs/batch_size)+1):
            ...
            codings_val = codings.eval(feed_dict={X: list(features)})
            new_features += (zip(labels, codings_val))

        with open("newnewfeatures.pkl", "wb") as f:
